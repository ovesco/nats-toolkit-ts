...
class SubscriberManager<S extends SubscriberDefinitions, C extends {} = {}> extends BaseManager<C> {
  ...
  async subscribe<K extends keyof S>(subject: K, subscriber: Subscriber<TypeOf<S[K]>, C>, opts?: SubscriptionOptions): Promise<Subscription> {
    ...
    (async (subscription) => {
      for await (const message of subscription) {
        this.withBasicPayload<Msg, SubscribeEnvelope<TypeOf<S[K]>>>(subject as string, 'subscribe', message, async (payload) => {
          super.validateEnvelope(payload.envelope, { data: payload.envelope.data, schema: this.subscriptions[subject] }, undefined, true);
          await subscriber({
            ...payload,
  ...
  async publish<K extends keyof S>(subject: K, message: SendPayload<TypeOf<S[K]>>, publishSpan?: Span) {
    ...
    const validation = super.validateEnvelope(envelope, { data: envelope.data, schema: this.subscriptions[subject] }, undefined);
    if (validation !== true) {
